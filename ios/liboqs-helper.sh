...
build_iphoneos() {
    LIB_NAME="liboqs.a"
    cmake -G Xcode -B ${PREFIX}/build-iphoneos -DPLATFORM=OS64 -DDEPLOYMENT_TARGET=$MIN_IOS_VERSION -DCMAKE_TOOLCHAIN_FILE=apple.cmake -DOQS_BUILD_ONLY_LIB=ON -DOQS_USE_OPENSSL=OFF .
...
build_iphonesimulator() {
    LIB_NAME="liboqs.a"
    cmake -G Xcode -B ${PREFIX}/build-iphonesimulator -DPLATFORM=SIMULATOR64 -DDEPLOYMENT_TARGET=$MIN_IOS_VERSION -DCMAKE_TOOLCHAIN_FILE=apple.cmake -DOQS_BUILD_ONLY_LIB=ON -DOQS_USE_OPENSSL=OFF .
...
build_tvos() {
    LIB_NAME="liboqs.a"
    cmake -G Xcode -B ${PREFIX}/build-appletvos -DPLATFORM=TVOS -DDEPLOYMENT_TARGET=$MIN_TVOS_VERSION -DCMAKE_TOOLCHAIN_FILE=apple.cmake -DOQS_BUILD_ONLY_LIB=ON -DOQS_USE_OPENSSL=OFF .
...
build_tvsimulator() {
    LIB_NAME="liboqs.a"
    cmake -G Xcode -B ${PREFIX}/build-appletvsimulator -DPLATFORM=SIMULATOR_TVOS -DDEPLOYMENT_TARGET=$MIN_TVOS_VERSION -DCMAKE_TOOLCHAIN_FILE=apple.cmake -DOQS_BUILD_ONLY_LIB=ON -DOQS_USE_OPENSSL=OFF .
