...
thread_local uint8_t write_buffer[HE_MAX_WIRE_MTU];
...
int he_wolf_dtls_read(WOLFSSL *ssl, char *buf, int sz, void *ctx) {
  ...
  if(!he_internal_is_pkt_available()) {
...
int he_wolf_dtls_write(WOLFSSL *ssl, char *buf, int sz, void *ctx) {
  ...
  if(sz + sizeof(he_wire_hdr_t) > sizeof(write_buffer)) {
  ...
  he_internal_write_packet_header(conn, (he_wire_hdr_t *)write_buffer);

  // Copy in the data behind the header
  // TODO Actively investigating why the analyzer thinks that write_buffer is not the same
  // as &write_buffer[0]
  memcpy((&write_buffer[0]) + sizeof(he_wire_hdr_t), buf, sz);
  ...
  he_return_code_t res = he_plugin_egress(conn->outside_plugins, &write_buffer[0],
                                          &post_plugin_length, sizeof(write_buffer));

  if(res == HE_ERR_PLUGIN_DROP) {
    // Plugin said to drop it, we drop it
    // Parallel to returning HE_SUCCESS on ingress
    return sz;
  } else if(res != HE_SUCCESS || post_plugin_length > sizeof(write_buffer)) {
    return WOLFSSL_CBIO_ERR_GENERAL;
  }

  // Call the write callback if set
  if(conn->outside_write_cb) {
    res = conn->outside_write_cb(conn, write_buffer, post_plugin_length, conn->data);
    if(res != HE_SUCCESS) {
      return WOLFSSL_CBIO_ERR_GENERAL;
    }
...
int he_wolf_tls_write(WOLFSSL *ssl, char *buf, int sz, void *ctx) {
  ...
  // Figure out how much to copy
  if(sz < sizeof(write_buffer)) {
    number_of_bytes_to_copy = sz;
  } else {
    number_of_bytes_to_copy = sizeof(write_buffer);
  }

  // Copy in the data
  memcpy((&write_buffer[0]), buf, number_of_bytes_to_copy);
