...
void test_he_internal_pmtud_send_probe(void) {
  ...
  TEST_ASSERT_EQUAL(probe_mtu, conn.pmtud.probing_size);
...
void test_he_internal_pmtud_send_probe_failed(void) {
  ...
  TEST_ASSERT_EQUAL(1, call_counter);
  TEST_ASSERT_EQUAL(probe_mtu, conn.pmtud.probing_size);
  TEST_ASSERT_EQUAL(0, conn.pmtud.probe_pending_id);
}
...
void test_he_internal_pmtud_handle_probe_ack_invalid_id(void) {
  ...
  conn.pmtud.probe_pending_id = 123;
...
void test_he_internal_pmtud_handle_probe_ack_from_base(void) {
  ...
  TEST_ASSERT_EQUAL(HE_PMTUD_STATE_SEARCHING, conn.pmtud.state);
  // Should be using big step when first entering searching state
  TEST_ASSERT_TRUE(conn.pmtud.is_using_big_step);
  // Current probing size should be BASE_PMTU + BIG_STEP
  TEST_ASSERT_EQUAL(conn.pmtud.base + PMTUD_PROBE_BIG_STEP, conn.pmtud.probing_size);
...
void test_he_internal_pmtud_handle_probe_ack_from_error(void) {
  conn.state = HE_STATE_ONLINE;
  conn.pmtud.state = HE_PMTUD_STATE_ERROR;
  conn.pmtud.probe_pending_id = 123;
  conn.pmtud_state_change_cb = pmtud_state_change_cb;
  conn.pmtud_time_cb = pmtud_time_cb;
  conn.pmtud.base = HE_MAX_MTU;
...
static void test_handle_probe_ack_from_searching(uint16_t probe_size, bool use_big_step) {
  ...
  conn.pmtud.probing_size = probe_size;
  conn.pmtud.is_using_big_step = use_big_step;
  ...
  if(probe_size >= MAX_PLPMTU) {
    ...
    TEST_ASSERT_EQUAL(HE_PMTUD_STATE_SEARCH_COMPLETE, conn.pmtud.state);
    // Effective PMTU should be set
    TEST_ASSERT_EQUAL(MAX_PLPMTU, conn.pmtud.effective_pmtu);
    ...
    if(use_big_step) {
      TEST_ASSERT_EQUAL(probe_size + PMTUD_PROBE_BIG_STEP, conn.pmtud.probing_size);
    } else {
      TEST_ASSERT_EQUAL(probe_size + PMTUD_PROBE_SMALL_STEP, conn.pmtud.probing_size);
    }
...
void test_he_internal_pmtud_handle_probe_timeout_try_again(void) {
  conn.state = HE_STATE_ONLINE;
  conn.pmtud.state = HE_PMTUD_STATE_SEARCHING;
  conn.pmtud.probe_count = 1;
  conn.pmtud.probing_size = MAX_PLPMTU - 120;
  conn.pmtud_time_cb = pmtud_time_cb;
  ...
  TEST_ASSERT_EQUAL(2, conn.pmtud.probe_count);
  TEST_ASSERT_EQUAL(43, conn.pmtud.probe_pending_id);
...
void test_he_internal_pmtud_handle_probe_timeout_confirm_base_failed(void) {
  ...
  conn.pmtud_time_cb = pmtud_time_cb;
  conn.pmtud.probing_size = MIN_PLPMTU;
  ...
  TEST_ASSERT_EQUAL(HE_PMTUD_STATE_ERROR, conn.pmtud.state);

  // The probe count and pending id should be reset to 0
  TEST_ASSERT_EQUAL(0, conn.pmtud.probe_count);
  TEST_ASSERT_EQUAL(0, conn.pmtud.probe_pending_id);
...
void test_he_internal_pmtud_handle_probe_timeout_confirm_base_retry_with_min_plpmtu(void) {
  ...
  TEST_ASSERT_EQUAL(HE_PMTUD_STATE_BASE, conn.pmtud.state);
...
void test_he_internal_pmtud_start_base_probing(void) {
  conn.state = HE_STATE_ONLINE;
  conn.pmtud.state = HE_PMTUD_STATE_DISABLED;
  conn.pmtud_time_cb = pmtud_time_cb;
  conn.ping_next_id = 42;
  ...
  TEST_ASSERT_EQUAL(INITIAL_PLPMTU, conn.pmtud.probing_size);
...
void test_he_internal_pmtud_handle_probe_timeout_search_completed(void) {
  ...
  TEST_ASSERT_EQUAL(MAX_PLPMTU - 120 - PMTUD_PROBE_SMALL_STEP, conn.pmtud.effective_pmtu);
